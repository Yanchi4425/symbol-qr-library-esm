...
import { CosignatureQR, ITransaction, QRCodeType } from '../index';
...
describe('CosignatureQR -->', () => {
  ...
  describe('fromJSON() should', () => {
    it('reconstruct aggregate bonded given CosignatureQR JSON', () => {
      ...
      // Act:
      const exportCosig = new CosignatureQR(
        bonded,
        NetworkType.TEST_NET,
        'no-chain-id'
      );
      const importCosig = CosignatureQR.fromJSON(
        exportCosig.toJSON(),
        TransactionMapping.createFromPayload
      );

      // Assert
      expect(importCosig.transaction.serialize()).to.be.equal(
        exportCosig.transaction.serialize()
      );
    });
    ...
    it('reconstruct aggregate bonded given correct JSON structure', () => {
      ...
      // Act:
      const importCosig = CosignatureQR.fromJSON(
        JSON.stringify(cosigInfo),
        TransactionMapping.createFromPayload
      );

      // Assert:
      expect(importCosig.transaction.serialize()).to.be.equal(
        cosigInfo.data.payload
      );
    });
  });
});
