...
const generationHash =
  '17FA4747F5014B50413CCF968749604D728D7065DC504291EEE556899A534CBB';
...
describe('QRCodeGenerator -->', () => {
  ...
  describe('createTransactionRequest() should', () => {
    it('generate correct Base64 representation for TransferTransaction', async () => {
      ...
      // Act:
      const requestTx = QRCodeGenerator.createTransactionRequest(
        transfer,
        networkType,
        generationHash
      );
  ...
  describe('createExportMnemonic() should', () => {
    it('generate correct Base64 representation for ExportMnemonic', async () => {
      ...
      // Act:
      const exportMnemonic = QRCodeGenerator.createExportMnemonic(
        mnemonic.plain,
        networkType,
        generationHash,
        'password'
  ...
  describe('fromJSON() should', () => {
    it('Populate transaction data given TransactionQR JSON', () => {
      ...
      expect(transactionObj.transaction.serialize()).to.be.equal(
        transfer.serialize()
      );
    });
    ...
    it('Populate account information given AccountQR JSON', () => {
      ...
      // Act:
      const accountObj: AccountQR = QRCodeGenerator.fromJSON(
        actualObj,
        TransactionMapping.createFromPayload,
        'password'
      ) as AccountQR;
      ...
      expect(
        Account.createFromPrivateKey(accountObj.accountPrivateKey, networkType)
    ...
    it('Populate object given ObjectQR JSON', () => {
      ...
      const exportObject = QRCodeGenerator.createExportObject(
        object,
        networkType,
        generationHash
      );
      ...
      // Act:
      const objectObj: ObjectQR = QRCodeGenerator.fromJSON(
        actualObj,
        TransactionMapping.createFromPayload
      ) as ObjectQR;
    ...
    it('Populate mnemonic pass phrase given MnemonicQR JSON', () => {
      ...
      // Act:
      const mnemonicObj: MnemonicQR = QRCodeGenerator.fromJSON(
        actualObj,
        TransactionMapping.createFromPayload,
        'password'
      ) as MnemonicQR;
      ...
      // create mnemonic
      const exportedMnemonic = new MnemonicPassPhrase(
        mnemonicObj.mnemonicPlainText
      );
    ...
    it('Populate signed transaction QR', () => {
      ...
      // Act:
      const signedObj: SignedTransactionQR = QRCodeGenerator.fromJSON(
        actualObj,
        undefined,
        undefined,
        mapper
      ) as SignedTransactionQR;
      ...
      // Assert:
      expect(qr.singedTransaction.toDTO().payload).to.be.equal(
    ...
    it('Populate cosignature signed transaction QR', () => {
      ...
      // Assert:
      expect(qr.singedTransaction.signerPublicKey).to.be.equal(
