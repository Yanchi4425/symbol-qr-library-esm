...
import { CosignatureSignedTransaction, NetworkType } from 'symbol-sdk';
...
describe('CosignatureSignedTransactionQR -->', () => {
  describe('toJSON() should', () => {
    it('include mandatory NIP-7 QR Code base fields', () => {
      ...
      // Act:
      const signedTx = new CosignatureSignedTransactionQR(
        signedTransaction,
  ...
  describe('fromJSON() should', () => {
    it('reconstruct signed transaction', () => {
      ...
      // Act:
      const reconstructedQR = CosignatureSignedTransactionQR.fromJSON(
        qr.toJSON(),
        mapper
      );

      // Assert
      expect(qr.singedTransaction.signature).to.be.equal(
        reconstructedQR.singedTransaction.signature
      );
    });
  });
});
