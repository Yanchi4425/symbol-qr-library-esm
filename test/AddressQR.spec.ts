...
import { AddressQR, QRCodeType } from '../index';
...
describe('AccountQR -->', () => {
  describe('toJSON() should', () => {
    it('include mandatory NIP-7 QR Code base fields', () => {
      ...
      const address = Address.createFromRawAddress(
        'TA6QZTYPOIYQYR5NRY4WQ2WRQUX2FN5UK2DO6DI'
      );
  ...
  describe('fromJSON() should', () => {
    it('reconstruct contact given AddressQR JSON', () => {
      ...
      // Act:
      const exportContact = new AddressQR(
        'nemtech',
        address.plain(),
        NetworkType.TEST_NET,
        'no-chain-id'
      ...
      expect(importContact.accountAddress).to.be.equal(
        exportContact.accountAddress
      );
    });
    ...
    it('reconstruct contact given correct JSON structure', () => {
      ...
      expect(addressQR.accountAddress).to.be.equal(
        'TA6QZTYPOIYQYR5NRY4WQ2WRQUX2FN5UK2DO6DI'
      );
    });
  });
